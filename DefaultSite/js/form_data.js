window.FormDataCompatibility = (function() {
  ...
  FormDataCompatibility.prototype.buildBody = function(encode) {
    ...
    for (key in _ref) {
      value = _ref[key];
      parts.push(this.buildPart(key, value, encode));
    }
  ...
  FormDataCompatibility.prototype.buildPart = function(key, value, encode) {
    var part;
    var encodeValue = encode ? (function(v) { return unescape(encodeURIComponent(v)); }) : (function(v) { return v; });
    if (typeof value === "string") {
      part = "Content-Disposition: form-data; name=\"" + key + "\"" + this.CRLF;
      part += "Content-Type: text/plain; charset=utf-8" + this.CRLF + this.CRLF;
      part += encodeValue(value) + this.CRLF;	// UTF-8 encoded like in real FormData
      ...
      if (value.type == 'file') {
        ...
        part = "Content-Disposition: form-data; name=\"" + key + "\"" + this.CRLF;
        part += "Content-Type: text/plain; charset=utf-8" + this.CRLF + this.CRLF;
        part += encodeValue(value.value) + this.CRLF;	// UTF-8 encoded like in real FormData
      }
